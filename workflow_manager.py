# core/workflow_manager.py (النسخة الكاملة مع خطوط إنتاج متخصصة ودعم المدخلات المتعددة)
import logging
...
from ingestion.ingestion_engine import MultimediaIngestionEngine, InputType, ingestion_engine
    ...
    async def _handle_user_feedback(
        ...
        if user_feedback_fn and step_name in user_config.get("user_checkpoints", []):
            ...
            if isinstance(user_action, dict): # User provided a modified version
                ...
                # إثارة خطأ مخصص لإعادة تشغيل الخطوة أو السير
