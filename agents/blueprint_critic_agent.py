# agents/blueprint_critic_agent.py
"""
BlueprintCriticAgent (ناقد المخططات)
يقوم بمراجعة وتقييم المخططات السردية لضمان جودتها وتماسكها.
"""
from typing import Dict, Any, List
import logging

from .base_agent import BaseAgent
# نفترض أن نماذج البيانات هذه موجودة في ملف منفصل أو في blueprint_architect_agent
from agents.blueprint_architect_agent import StoryBlueprint

logger = logging.getLogger("BlueprintCriticAgent")

class BlueprintCriticAgent(BaseAgent):
    """
    ...
    def __init__(self, agent_id: Optional[str] = None):
        super().__init__(
            agent_id=agent_id,
            name="ناقد المخططات",
            description="يراجع المخططات السردية لضمان التماسك الدرامي والمنطقي."
        )

    def review_blueprint(self, blueprint: StoryBlueprint) -> Dict[str, Any]:
        """
        يراجع مخطط سردي ويعطي تقييمًا وملاحظات.
        """
        logger.info(f"Reviewing blueprint for '{blueprint.introduction[:30]}...'")
        ...
        if emotional_gaps:
            issues.append(f"الفصول التالية تحتاج لتعميق عاطفي: {', '.join(emotional_gaps)}. يجب تحديد المشاعر السائدة.")
        else:
